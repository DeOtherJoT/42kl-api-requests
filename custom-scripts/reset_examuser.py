...
# This script is meant to delete an ExamUser from an Exam and
# re-register him/her. Use this script when a student has registered
# to an Exam, but somehow cannot log in for whatever reason.
...
# First, list exams and list down the registered Cadets.
...
for user in exam_users:
	...
	user_options.append([user['user']['login'], user['id'], user['user']['id']])
	count += 1
...
if del_req.status_code != 204:
	...
	raise Exception("Error occured while deleting exam user.")

# Now that the student is deleted, time to add him/her back in
payload = {
	"exams_user": {
		"user_id": exam_user[2]
	}
...
post_req = oauth.post(f"{SITE}/v2/exams/{exam_id}/exams_users", json=payload)
if (post_req.status_code != 201):
	print(f"{Fore.RED}[ ERROR ] - Returned code {post_req.status_code}\n[ ERROR ] - {del_req.text}")
	raise Exception("Error pccured while adding exam user.")
